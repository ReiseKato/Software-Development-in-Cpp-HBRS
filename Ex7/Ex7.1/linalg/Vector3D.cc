...
namespace linalg
{
    ...
    {
        Vector3D::Coord x_conj = v.x().conjugate();
        Vector3D::Coord y_conj = v.y().conjugate();
        Vector3D::Coord z_conj = v.z().conjugate();
        return std::sqrt(x_conj * v.x() + y_conj * v.y() + z_conj * v.z());
    }
