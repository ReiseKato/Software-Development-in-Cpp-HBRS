...
For Vector2D, provide
- operator `-=` that subtracts a vector from a vector, `/=` that divides a vector by a scalar (`Coord`) value, and `*=` that multiplies a vector by a scalar
- binary operators `+`, `-`, `/`, and `*`, the latter of which can be applied to two vectors (scalar product) or to a scalar and a vector or to the reverse
- unary operator `-`
- operator `[]` that takes an index (natural number) and returns the corresponding coordinate by reference

making them global functions, if possible. Use the qualifier `const` and passing by reference, where possible.
